...
router.get('/cart/:id', async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT * from cart_item WHERE cart_id = $1`, [basketId]); // Requête SQL
...
router.post('/cart/:id/add', async(req, res)=>{
    try{
        ...
        if (existItem.rows.length > 0){
            ...
            await pool.query(
                `INSERT INTO cart_item (product_id, cart_id, quantity, price) VALUES ($1, $2, $3, $4)`,
